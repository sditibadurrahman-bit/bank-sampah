...
export default function AdminLayout() {
  const { appUser, currentUser: firebaseUser, logout } = useAuth();
  const currentUser = appUser;
  const location = useLocation();
  const navigate = useNavigate();
  const [showLogoutConfirm, setShowLogoutConfirm] = useState(false);

  if (!firebaseUser || !currentUser || currentUser.role !== 'admin') {
      ...
      <header className="bg-emerald-600 border-b border-emerald-700 px-4 py-4 sticky top-0 z-50 flex items-center justify-between shadow-md">
        <div className="flex items-center space-x-3">
          ...
          <h1 className="text-xl font-bold text-white tracking-tight">
            {isDashboard ? 'EcoBank Admin' : location.pathname.split('/').pop()}
          </h1>
        </div>
        ...
        <div className="flex items-center space-x-4">
          <div className="hidden sm:block text-right">
            ...
            <p className="text-[10px] bg-emerald-500 text-white px-2 py-0.5 rounded-full capitalize font-medium inline-block mt-1">{currentUser.role}</p>
          </div>
          <button 
